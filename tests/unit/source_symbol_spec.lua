...
describe('source_symbol', function()
  ...
  describe('is_static', function()
    it('returns true for static keyword in detail', function()
      local sym = SourceSymbol.new(mock_sym('foo', SK.Function, { detail = 'static int' }), 'file:///test.hpp', nil)
      assert.is_true(sym:is_static())
    end)
    it('returns false without static keyword in detail', function()
      local sym = SourceSymbol.new(mock_sym('foo', SK.Function, { detail = 'int' }), 'file:///test.hpp', nil)
      assert.is_false(sym:is_static())
    end)
    it('returns false for nil detail', function()
      local sym = SourceSymbol.new(mock_sym('foo', SK.Function), 'file:///test.hpp', nil)
      assert.is_false(sym:is_static())
    end)
  end)
  ...
  describe('is_variable', function()
    it('returns true for Variable kind outside class', function()
      local sym = SourceSymbol.new(mock_sym('x', SK.Variable), 'file:///test.hpp', nil)
