...
describe('function_signature', function()
  describe('new', function()
    ...
    it('handles constructor with no return type', function()
      local sig = FunctionSignature.new('MyClass()')
      eq('', sig.return_type)
      eq('MyClass', sig.name)
    end)
    ...
    it('parses complex return type with template reference', function()
      local sig = FunctionSignature.new('const std::vector<int>& foo()')
      eq('const std::vector<int>&', sig.return_type)
      eq('foo', sig.name)
    end)

    it('handles operator overload', function()
      local sig = FunctionSignature.new('bool operator==(const T& other)')
      eq('bool', sig.return_type)
      eq('operator==', sig.name)
      eq('const T& other', sig.parameters)
    end)

    it('handles function pointer parameter', function()
      local sig = FunctionSignature.new('void (*fp)(int)')
      eq('void', sig.return_type)
      eq('*fp', sig.parameters)
    end)

    it('handles deeply nested parentheses', function()
      local sig = FunctionSignature.new('foo(bar(baz))')
      eq('bar(baz)', sig.parameters)
    end)

    it('extracts const override trailing', function()
      local sig = FunctionSignature.new('int foo() const override;')
      eq('const override;', sig.trailing)
    end)

    it('extracts final trailing', function()
      local sig = FunctionSignature.new('int foo() final')
      eq('final', sig.trailing)
    end)

    it('returns empty trailing for function with no qualifiers', function()
      local sig = FunctionSignature.new('void bar()')
      eq('', sig.trailing)
    end)
  end)
  ...
  describe('equals', function()
    ...
    it('returns false when trailing differs', function()
      local a = FunctionSignature.new('void foo() const;')
      local b = FunctionSignature.new('void foo() override;')
      assert.is_false(a:equals(b))
    end)

    it('handles nil comparison correctly', function()
      local a = FunctionSignature.new('void foo(int x);')
      local result = a:equals(nil)
